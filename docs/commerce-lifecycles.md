...
| Offline scan → unchanged/OFFLINE_PENDING | Signed, unexpired OfflineManifest permits provisional admission and local spent-token set has no match | Queue signed OfflineCommand for server resolution | Device sequence + request ID; cross-device conflicts surface for review (`OQ-021`) |
...
Offline sale is allowed only against a pre-issued, non-overlapping `OfflineCapacityBudget` assigned to a registered device and Session. The device may issue locally signed cash Orders/Tickets until the budget or signed manifest expires. Card-present sale is allowed only when the selected provider/reader contract supports offline/store-and-forward operation and the owner-approved risk limits are configured; local acceptance is provisional until provider settlement. Refunds, shared GiftCard value changes, membership changes, role/configuration changes, and exports are online-only.

On reconnection, commands upload in device-sequence order but the server remains tolerant of duplicate and delayed delivery. Each command resolves to `ACCEPTED`, `DUPLICATE`, `CONFLICT`, `EXPIRED_POLICY`, or `MANUAL_REVIEW`; accepted capacity consumes the device budget exactly once. Cross-device duplicate check-ins and any payment rejection remain visible exceptions and never overwrite earlier evidence. The power, UPS, battery, queue, freshness, and synchronization envelope is specified in [Venue operations and offline mode](operations-integrations-and-reporting.md#offline-and-disruption-operating-model).
