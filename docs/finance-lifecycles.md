...
The launch venue is the single initial merchant and payment collector. The Stripe adapter uses one approved merchant account context; Stripe Connect, connected accounts, transfers, and marketplace/seller payouts are excluded from the initial architecture. Location, channel, and Terminal reader references remain separate from merchant identity. Merchant/seller/tax treatment and supported currencies still require legal/accounting confirmation (`OQ-006`, `OQ-008`).
...
| REQUESTED → APPROVAL_REQUIRED/APPROVED | Evaluate amount, role, separation of duties, service/check-in policy | Expected version; approval actor differs when required (`OQ-014`) |
...
Mixed-tender refund allocation is policy-driven and versioned; no allocation may exceed original tender capture. Policy is **UNVERIFIED (OQ-015)**.
...
| ACTIVE/SUSPENDED → EXPIRED | Approved expiry policy/date permits | Append EXPIRE and accounting treatment | Legal policy version; scheduled key (`OQ-007`) |
...
Dunning cadence, grace access, proration, pauses, credits, and cancellation timing are **UNVERIFIED (OQ-016)**.
...
Daily reconciliation matches internal captures/refunds/fees/disputes to the single merchant’s provider balance movements, groups them to payouts, and matches payouts to bank deposits. Exceptions include missing source/journal, duplicate, amount/currency mismatch, aged pending funds, unmatched payout/deposit, and unresolved negative balance. Webhooks provide timeliness; provider reports and bank facts provide completeness.
