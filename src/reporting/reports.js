...
export class ExportJobStore {
  ...
  create({ reportId, format = 'csv', filters = {}, idempotencyKey }) { assert(REPORT_DEFINITIONS[reportId] && ['csv', 'json'].includes(format) && filters && typeof filters === 'object' && !Array.isArray(filters) && idempotencyKey, 'VALIDATION_FAILED'); const fingerprint = JSON.stringify({ reportId, format, filters }); const prior = this.keys.get(idempotencyKey); if (prior) { if (prior.fingerprint !== fingerprint) throw new DomainError('IDEMPOTENCY_MISMATCH'); return prior.job; } const job = Object.freeze({ id: `export-${String(this.jobs.length + 1).padStart(4, '0')}`, reportId, format, filters: Object.freeze({ ...filters }), status: 'QUEUED', downloadable: false, createdAt: this.now(), providerCalls: 0 }); this.jobs.push(job); this.keys.set(idempotencyKey, { fingerprint, job }); return job; }
