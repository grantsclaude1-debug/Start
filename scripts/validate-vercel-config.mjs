...
if (Object.hasOwn(config.functions?.['api/handler.js'] ?? {}, 'runtime')) throw new Error('JavaScript functions must use Vercel\'s built-in Node.js runtime rather than an invalid runtime package string');
if (packageConfig.engines?.node !== '>=24') throw new Error('package.json must request Node.js 24 or newer');
...
console.log('PASS vercel config: built-in Node.js runtime with Node 24 engine, bundled public assets, guarded catch-all routing, and no embedded gate secrets');
